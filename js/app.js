...
function addMember(e) {
  e.preventDefault();
  if (!nameInput.value.match(pattern)&& nameInput.value === "") {
...
function clearAll() {
  ...
  team.innerHTML = '';
  memberList.innerHTML = '';
  teamsg.classList.add('show');
    setTimeout(() => {
      teamsg.classList.remove('show');
    }, 1700);
}
...
function saveToStorage() {
  ...
  if (!membersNames.length) {
    ...
    var query = `select * from table where id in ("${membersNames.join('", "')}")`;
    console.log(query);
  }
}
